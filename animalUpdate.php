...
<body>
    <div class="bordaImagemHome">
        <p> Atualizar as Informações do Anúncio </p>
        ...
        try{
            if (isset($_POST['submit'])) {
                ...
                if (empty($_POST['adUsername']) || empty($_POST['adPhone']) || empty($_POST['adEmail']) || empty($_POST['adState']) || empty($_POST['adCity']) || empty($_POST['adAnimalType']) || empty($_POST['adAnimalFeature'])) {
                    ...
                    $query = 'UPDATE anuncio SET anuncio_nome_completo=:nomeCompleto, anuncio_numero_telefone=:telefone, anuncio_email_contato=:email, anuncio_estado_local_animal=:estado, anuncio_cidade_local_animal=:cidade, anuncio_desc=:descricao, anuncio_cod_usuario=:cod_usuario WHERE cod_anuncio = :codAnuncio'; 
                    ...
                    if ($submitData -> execute()) {
                        ...
                        $submitData -> bindValue(':codAnuncioAnimal', $codAnuncio);
                        $submitData -> bindValue(':codAnimal', $_SESSION['codAnimal']);
